...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["image/png", "image/jpeg", "image/jpg", "image/webp"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for event-images" ON storage.objects FOR SELECT USING (bucket_id = 'event-images');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for event-images" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'event-images');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for event-images" ON storage.objects FOR UPDATE USING (bucket_id = 'event-images');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for event-images" ON storage.objects FOR DELETE USING (bucket_id = 'event-images');`
    ];
