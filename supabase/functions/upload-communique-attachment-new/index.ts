Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!allowedTypes.includes(file.type)) {
      return new Response(
        JSON.stringify({ error: 'Invalid file type. PDF, images, and Word documents allowed.' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    const uploadUrl = `${supabaseUrl}/storage/v1/object/communique-attachments/${fileName}`;
