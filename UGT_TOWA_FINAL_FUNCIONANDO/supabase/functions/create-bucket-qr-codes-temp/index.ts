...
Deno.serve(async (req) => {
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for qr-codes" ON storage.objects FOR SELECT USING (bucket_id = 'qr-codes');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for qr-codes" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'qr-codes');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for qr-codes" ON storage.objects FOR UPDATE USING (bucket_id = 'qr-codes');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for qr-codes" ON storage.objects FOR DELETE USING (bucket_id = 'qr-codes');`
    ];
