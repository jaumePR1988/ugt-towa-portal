...
<body>
    <div class="container">
        ...
        ${contentByType.surveys.length > 0 ? `
        <div class="section">
            <h2 class="section-title">Resultados de Encuestas</h2>
            ${contentByType.surveys.map(survey => `
                <div class="content-item">
                    <h3>${survey.title}</h3>
                    <p><strong>Total de participantes:</strong> ${survey.totalResponses}</p>
                    ${survey.statistics && survey.statistics.options ? `
                        <div style="margin-top: 15px;">
                            ${survey.statistics.options.map(option => `
                                <div style="margin-bottom: 10px; background-color: #f0f0f0; padding: 8px; border-radius: 4px;">
                                    <div style="display: flex; justify-content: space-between; align-items: center;">
                                        <span style="font-weight: bold;">${option.text}</span>
                                        <span style="color: #e50000; font-weight: bold;">${option.percentage}% (${option.votes} votos)</span>
                                    </div>
                                    <div style="background-color: #ddd; height: 8px; border-radius: 4px; margin-top: 5px;">
                                        <div style="background-color: #e50000; height: 100%; width: ${option.percentage}%; border-radius: 4px;"></div>
                                    </div>
                                </div>
                            `).join('')}
                        </div>
