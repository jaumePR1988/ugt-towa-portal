Deno.serve(async (req) => {
  ...
  try {
    ...
    const fileName = `appointment-doc-${timestamp}-${randomString}.${fileExtension}`;
    ...
    const uploadResponse = await fetch(
      `${supabaseUrl}/storage/v1/object/appointment-documents/${fileName}`,
      {
        method: 'POST',
        headers: {
    ...
    const fileUrl = `${supabaseUrl}/storage/v1/object/public/appointment-documents/${fileName}`;
    ...
    console.error('Error en upload-appointment-document:', error);
    return new Response(
      JSON.stringify({
