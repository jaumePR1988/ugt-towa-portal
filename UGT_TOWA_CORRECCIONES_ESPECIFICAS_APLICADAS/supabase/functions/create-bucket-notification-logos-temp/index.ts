...
Deno.serve(async (req) => {
    ...
    const bucketConfig = {
        id: 'notification-logos',
        name: 'notification-logos',
        public: true
    };

    // Add optional configurations
    bucketConfig.allowed_mime_types = ["image/png", "image/jpeg", "image/svg+xml", "image/webp"];
    bucketConfig.file_size_limit = 1048576;
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for notification-logos" ON storage.objects FOR SELECT USING (bucket_id = 'notification-logos');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for notification-logos" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'notification-logos');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for notification-logos" ON storage.objects FOR UPDATE USING (bucket_id = 'notification-logos');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for notification-logos" ON storage.objects FOR DELETE USING (bucket_id = 'notification-logos');`
    ];
