...
Deno.serve(async (req) => {
    ...
    const bucketConfig = {
        id: 'delegate-photos',
        name: 'delegate-photos',
        public: true
    };

    // Add optional configurations
    bucketConfig.allowed_mime_types = ["image/jpeg", "image/jpg", "image/png", "image/webp"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for delegate-photos" ON storage.objects FOR SELECT USING (bucket_id = 'delegate-photos');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for delegate-photos" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'delegate-photos');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for delegate-photos" ON storage.objects FOR UPDATE USING (bucket_id = 'delegate-photos');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for delegate-photos" ON storage.objects FOR DELETE USING (bucket_id = 'delegate-photos');`
    ];
